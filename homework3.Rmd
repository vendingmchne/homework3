...
gss16_tidyfit_1 <- gss16_fit_1 %>% tidy()
...
```{r logroc1}
gss16_pred_1 <- predict(gss16_fit_1, gss16_test, type = "prob") %>% bind_cols(gss16_test)
gss16_pred_1

gss16_roccurve_1 <- gss16_pred_1 %>% roc_curve(
  truth = advfront, .pred_Agree, event_level = "second") %>%
  autoplot()
gss16_roccurve_1
...
gss16_tidyfit_2 <- gss16_fit_2 %>% tidy()

gss16_pred_2 <- predict(gss16_fit_2, gss16_test, type = "prob") %>% bind_cols(gss16_test)
gss16_pred_2

gss16_roccurve_2 <- gss16_pred_2 %>% roc_curve(
  truth = advfront, .pred_Agree, event_level = "second") %>%
  autoplot()
...
gss16_auc_1 <- gss16_pred_1 %>% roc_auc(
  truth = advfront, .pred_Agree, event_level = "second") %>% print()

gss16_auc_2 <- gss16_pred_2 %>% roc_auc(
  truth = advfront, .pred_Agree, event_level = "second") %>% print()
```
...
Model 1's area under the ROC curve is larger than the area under the ROC curve for Model 2. This means that its predictions are more likely to be true than those of Model 2. This means that education level alone is a better predictor than the combination of education level, political views and working status.
